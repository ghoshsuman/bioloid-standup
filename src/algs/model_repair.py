...
from models.DTMC import state_mapper
...
class DeltaRepairer(LocalRepairer):
    ...
    def repair(self, dtmc_generator, probs):
        prob = stormpy.ExplicitQuantitativeCheckResult.at(probs,state_mapper.INITIAL_STATE)
        ...
        repair = False
        for state in range(len(stormpy.ExplicitQuantitativeCheckResult.get_values(probs))):
    ...
    def repair_state(self, dtmc_generator, probs, state, delta):
        ...
        for action in possible_actions:
            ...
            for (succ_state, prob) in successors:
                p += prob * stormpy.ExplicitQuantitativeCheckResult.at(probs,succ_state)
